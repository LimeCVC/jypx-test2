--[[
    JYPX // V1.0
    Исправленная версия с рабочими вкладками, ресайзом и кнопкой свернуть
]]
...
local function stylizeButton(btn, bgColor)
...
stylizeButton(MinBtn, Colors.Panel)
...
stylizeButton(CloseBtn, Colors.Panel)
...
UserInputService.InputChanged:Connect(function(input)
    ...
    for _, child in pairs(MainFrame:GetDescendants()) do
        if child.Name == "ContentArea" or child.Name == "TabsBar" then
            pcall(function()
                child.Size = UDim2.new(0, 100, 1, -60)
            end)
        end
        if child.Name == "subContent" then
            pcall(function()
                child.Size = UDim2.new(1, 0, 1, -38)
            end)
        end
    end
end)
...
UserInputService.InputEnded:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        isResizing = false
        Settings.windowWidth = MainFrame.AbsoluteSize.X
        Settings.windowHeight = MainFrame.AbsoluteSize.Y
        saveSettings()
    end
end)
...
local function createTab(name, label)
    ...
    btn.Parent = TabsBar
    stylizeButton(btn, Colors.PanelElevated)
...
local function createInput(placeholder, parent, callback)
    ...
    inp.Parent = parent
    stylizeButton(inp, Colors.PanelSoft)
...
local function createSlider(label, minVal, maxVal, defaultVal, parent, callback)
    ...
    stylizeButton(slider, Colors.PanelElevated)
    ...
    fill.Parent = slider
    stylizeButton(fill, Colors.ActiveBG)
...
stylizeButton(statusLabel, Colors.PanelSoft)
...
local function createSubTab(label, parent)
    local btn = Instance.new("TextButton")
    ...
    stylizeButton(btn, Colors.PanelSoft)
